...
class AlarmClockCard extends AlarmClockBase {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `${this.styles()}<ha-card><h2><ha-icon class="${enabled ? "enabled" : "disabled"}" icon="${esc(this.config.icon || "mdi:alarm")}"></ha-icon><span class="heading"><span>${esc(this.config.title || this.config.name || "Alarm Clock")}</span><span class="sub">${esc(friendlyNext())}</span></span></h2><div class="grid">${schedule.map(([label,key]) => `<div class="box"><label>${label}</label><br><b>${esc((this.value(key) || "--:--").slice(0,5))}</b></div>`).join("")}</div><div class="row"><span>Override ${esc((this.value("override_time") || "--:--").slice(0,5))}</span><button id="open">${this.config.navigation_path ? "Open alarm" : "Details"}</button></div></ha-card>`;
