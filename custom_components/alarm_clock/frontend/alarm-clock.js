...
class AlarmClockEditorCard extends AlarmClockBase {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `${this.styles()}<ha-card><h2>⏰ ${esc(this.config.name || "Alarm Clock")}</h2><div class="sub">${esc(this.value("status") || "unknown")}</div>${toggle("Enabled","enabled")}${toggle("One-shot override","override")}<div class="grid">${schedule}${clock("Override time","override_time")}</div><button id="advanced">${advanced?"Hide":"Show"} advanced settings</button><section class="${advanced?"":"hidden"}">${toggle("Primary pre-alarm","primary_pre_enabled")}${num("Primary pre-alarm volume (%)","primary_pre_volume")}${num("Primary pre-alarm duration (seconds)","primary_pre_duration")}${num("Primary pre-alarm repeat (seconds)","primary_pre_repeat")}${num("Primary main volume (%)","primary_main_volume")}${toggle("Follow-up","followup_enabled")}${num("Follow-up delay (minutes)","followup_delay")}${toggle("Follow-up pre-alarm","followup_pre_enabled")}${num("Follow-up pre-alarm volume (%)","followup_pre_volume")}${num("Follow-up pre-alarm duration (seconds)","followup_pre_duration")}${num("Follow-up pre-alarm repeat (seconds)","followup_pre_repeat")}${toggle("Reuse primary main media","followup_reuse_primary")}${num("Follow-up main volume (%)","followup_main_volume")}${num("Stop after (minutes)","stop_after")}<div class="row"><span>Media and schedule mode are set in the integration Configure screen.</span><button id="configure">Configure</button></div></section></ha-card>`;
