...
class AlarmClockEditorCard extends AlarmClockBase {
  render() {
    ...
    const daily = mode === "per_day" ? `<div class="grid">${days.map(day=>`<div class="box"><label>${day[0].toUpperCase()+day.slice(1)}</label><br><input data-toggle="${day}_enabled" type="checkbox" ${this.value(`${day}_enabled`)==="on"?"checked":""}><input data-time="${day}_time" type="time" value="${esc((this.value(`${day}_time`)||"").slice(0,5))}"></div>`).join("")}</div>` : "";
    this.shadowRoot.innerHTML = `${this.styles()}<ha-card><h2>⏰ ${esc(this.config.name || "Alarm Clock")}</h2><div class="sub">${esc(this.value("status") || "unknown")}</div>${toggle("Enabled","enabled")}${toggle("One-shot override","override")}<div class="grid">${compact}${clock("Override time","override_time")}</div><button id="advanced">${advanced?"Hide":"Show"} advanced settings</button><section class="${advanced?"":"hidden"}"><div class="row"><label>Schedule mode</label><select data-select="schedule_mode"><option value="compact" ${mode==="compact"?"selected":""}>Weekday / Sat / Sun</option><option value="per_day" ${mode==="per_day"?"selected":""}>Individual days</option></select></div>${daily}${toggle("Primary pre-alarm","primary_pre_enabled")}${num("Primary pre-alarm volume (%)","primary_pre_volume")}${num("Primary pre-alarm duration (seconds)","primary_pre_duration")}${num("Primary pre-alarm repeat (seconds)","primary_pre_repeat")}${num("Primary main volume (%)","primary_main_volume")}${toggle("Follow-up","followup_enabled")}${num("Follow-up delay (minutes)","followup_delay")}${toggle("Follow-up pre-alarm","followup_pre_enabled")}${num("Follow-up pre-alarm volume (%)","followup_pre_volume")}${num("Follow-up pre-alarm duration (seconds)","followup_pre_duration")}${num("Follow-up pre-alarm repeat (seconds)","followup_pre_repeat")}${toggle("Reuse primary main media","followup_reuse_primary")}${num("Follow-up main volume (%)","followup_main_volume")}${num("Stop after (minutes)","stop_after")}<div class="row"><span>Media is selected in the integration Configure screen.</span><button id="configure">Configure media</button></div></section></ha-card>`;
