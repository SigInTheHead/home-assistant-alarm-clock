...
class AlarmClockBase extends HTMLElement {
  ...
  styles() { return `<style>:host{display:block}ha-card{padding:16px}h2{display:flex;align-items:center;gap:8px;margin:0}.heading{display:flex;flex-direction:column;gap:2px}.heading .sub{font-weight:normal}h2 ha-icon{--mdc-icon-size:28px}h2 ha-icon.enabled{color:var(--primary-color)!important}h2 ha-icon.disabled{color:var(--disabled-text-color,#9e9e9e)!important}.summary{padding:12px}.summary h2{font-size:16px;line-height:1.2}.summary h2 ha-icon{--mdc-icon-size:24px}.summary .sub{font-size:12px}.sub,label{color:var(--secondary-text-color);font-size:13px}.row{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 0;border-top:1px solid var(--divider-color)}.override{border-top:0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(125px,1fr));gap:8px;margin:12px 0}.box{background:var(--secondary-background-color);border-radius:8px;padding:9px}.time-header{display:flex;align-items:center;justify-content:space-between;gap:8px}input,select,button{font:inherit;padding:6px;border-radius:6px;background:var(--secondary-background-color);color:var(--primary-text-color);border:1px solid var(--divider-color)}.time-controls{display:grid;grid-template-columns:1fr 14px 1fr;justify-items:center;align-items:center;margin-top:6px}.time-controls button{width:100%;padding:2px;border:0;background:transparent;font-size:18px;line-height:1;cursor:pointer}.time-value{font-size:20px;font-weight:600;padding:5px 0}.time-separator{font-size:18px}.hidden{display:none}</style>`; }
}
...
class AlarmClockEditorCardConfigEditor extends HTMLElement {
  ...
  render() {
    if (!this.shadowRoot || !this._config) return;
    this.shadowRoot.innerHTML = `<style>:host{display:block;padding:12px 0}.field{display:block}.field span{display:block;margin-bottom:6px;color:var(--primary-text-color);font-size:14px}.field small{display:block;margin-top:4px;color:var(--secondary-text-color)}ha-entity-picker{display:block;width:100%}</style><label class="field"><span>Entity</span><ha-entity-picker id="entity"></ha-entity-picker><small>Select the root Alarm Clock switch.</small></label>`;
    const entityPicker = this.shadowRoot.querySelector("#entity");
    entityPicker.hass = this._hass;
    entityPicker.value = this._config.entity || "";
    entityPicker.includeDomains = ["switch"];
...
class AlarmClockEditorCard extends AlarmClockBase {
  ...
  render() {
    ...
    const choice = (label,key) => {
      const item = this.find(key), options = item?.state.attributes?.options || [];
      return `<div class="row"><label>${label}</label><select data-select="${key}">${options.map((option) => `<option value="${esc(option)}"${this.value(key) === option ? " selected" : ""}>${esc(option)}</option>`).join("")}</select></div>`;
    };
    ...
    this.shadowRoot.innerHTML = `${this.styles()}<ha-card><h2>⏰ ${esc(this.config.name || "Alarm Clock")}</h2><div class="sub">${esc(this.value("status") || "unknown")}</div>${toggle("Enabled","enabled")}${toggle("One-shot override","override")}<div class="grid">${schedule}${clock("Override time","override_time")}</div><button id="advanced">${advanced?"Hide":"Show"} advanced settings</button><section class="${advanced?"":"hidden"}">${toggle("Primary pre-alarm","primary_pre_enabled")}${choice("Primary pre-alarm tone","primary_pre_media")}${num("Primary pre-alarm run for (seconds)","primary_pre_duration")}${num("Primary pre-alarm volume (%)","primary_pre_volume")}${num("Primary main volume (%)","primary_main_volume")}${toggle("Follow-up","followup_enabled")}${num("Follow-up delay (minutes)","followup_delay")}${toggle("Follow-up pre-alarm","followup_pre_enabled")}${choice("Follow-up pre-alarm tone","followup_pre_media")}${num("Follow-up pre-alarm run for (seconds)","followup_pre_duration")}${num("Follow-up pre-alarm volume (%)","followup_pre_volume")}${toggle("Reuse primary main media","followup_reuse_primary")}${num("Follow-up main volume (%)","followup_main_volume")}${num("Stop after (minutes)","stop_after")}<div class="row"><span>Main media and schedule mode are set in the integration Configure screen.</span><button id="configure">Configure</button></div></section></ha-card>`;
